...
const WriteModal: FC<WriteModalProps> = ({ onClose, onUpdate }) => {
  ...
  const handleFormSubmit: FormEventHandler<HTMLFormElement> = async (e) => {
    ...
    if (e.target instanceof HTMLFormElement && e.target.checkValidity()) {
      ...
      const data = (['password', 'name', 'content'] as const).reduce(
        (acc, value) => ({
          ...acc,
      ...
      const { postGuestBook } = await import('./firebasePlugin');
      ...
      <Styled.Background onClick={onClose}>
        <Styled.Paper tabIndex={-1} role="dialog" aria-modal="true" onClick={(e) => e.stopPropagation()}>
          ...
          <Styled.Form name="guest-book" onSubmit={handleFormSubmit}>
            ...
            <Styled.Input required type="text" id="name" name="name" autoComplete="no" />
            ...
            <Styled.TextArea required id="content" name="content" rows={10} />
            <div css={buttonArea}>
